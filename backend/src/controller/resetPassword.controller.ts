...
class resetPassword {
    ...
    public async validatesTokenHash(req: Request, res: Response) {
        try {
            ...
            if (String(hash) != String(user.resetToken)) {
                return res.status(500).send({ message: "Código de recuperção invalido!" });
            }

            if (user.dataReset <= (new Date())) {
                res.status(500).send({ message: "O código de recuperação expirou!" });
            }
    ...
    public async saveNewPassword(req: Request, res: Response) {
        try {
            ...
            return res.status(500).send({ message: "Senha atualizada com sucesso!" });
